...
singletonMem :: (Show i, Nominal i, Contextual i) => (Set [i] -> Set ([i], Formula)) -> [i] -> Formula
singletonMem oracle = snd . head . toList . oracle . singleton
...
teacherWithIO alph = Teacher
    { membership = ioMembership consts
    , equivalent = ioEquivalent consts (singletonMem (ioMembership consts))
    , alphabet   = alph
    , constants  = consts
    }
    where consts = unsafePerformIO $ newIORef []
...
teacherWithIO2 alph = Teacher
    { membership = ioMembership2 consts
    , equivalent = ioEquivalent2 consts (singletonMem (ioMembership2 consts))
    , alphabet   = alph
    , constants  = consts
    }
    where consts = unsafePerformIO $ newIORef []
...
teacherWithTargetAndIO aut = Teacher
    { membership = foreachQuery $ accepts aut
    , equivalent = ioEquivalent consts (accepts aut)
    , alphabet   = NLambda.alphabet aut
    , constants  = consts
    }
    where consts = unsafePerformIO $ newIORef []

automaticEquivalent :: (p1 -> p2 -> Set a) -> p1 -> p2 -> Either [Atom] (Maybe (Set a))
automaticEquivalent bisimlator aut hypo = case solve isEq of
        Nothing    -> error "should be solved"
        Just True  -> Right Nothing
        Just False -> Right (Just bisimRes)
        where
