...
teacherWithTarget aut = Teacher
    { membership = mqGeneraliser consts (accepts aut)
    , equivalent = eqGeneraliser consts (accepts aut) (automaticEquivalent bisim aut)
...
teacherWithTargetNonDet n aut = Teacher
    { membership = mqGeneraliser consts (accepts aut)
    , equivalent = eqGeneraliser consts (accepts aut) (automaticEquivalent (bisimNonDet n) aut)
    , alphabet   = NLambda.alphabet aut
    , constants  = consts
    }
    where consts = unsafePerformIO $ newIORef []
...
teacherWithIO alph = Teacher
    { membership = mqGeneraliser consts ioMembership
...
teacherWithIO2 alph = Teacher
    { membership = mqGeneraliser consts ioMembership2
...
teacherWithTargetAndIO aut = Teacher
    { membership = mqGeneraliser consts (accepts aut)
    , equivalent = eqGeneraliser consts (accepts aut) ioEquivalent
    , alphabet   = NLambda.alphabet aut
    , constants  = consts
    }
    where consts = unsafePerformIO $ newIORef []
